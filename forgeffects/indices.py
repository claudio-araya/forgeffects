...
def indices(min_result, maxmin_prima, thr):
    ...
    # Aqui obtengo un tensor unicamente con los valores mayores a 0
    valores_result = tf.boolean_mask(maxmin_prima, maxmin_prima > thr)

    # ----------------------------

    # Máscara para obtener los indices de las coordenadas donde los valores son mayores a 0
    mask_ind = tf.greater(maxmin_prima, thr)
    indices_fe = tf.where(mask_ind)

    # ----------------------------

    # Uso esos índices para obtener unicamente los valores de los minimos donde pasó energía (mayor a 0)
    ...
    ##### CON ESTO VERIFICO SI LOS INDICES SON MAYORES A 0, PERO QUEDA EVALUAR

    # # máscara booleana donde los valores son mayores a 0
    # greater_zero_mask = tf.greater(values, 0)

    # # combino ambas máscaras: el valor debe ser máximo y mayor a 0
    # final_mask = tf.logical_and(max_mask, greater_zero_mask)

    # # genero los índices para la última dimensión
    # indices = tf.range(start=0, limit=tf.shape(final_mask)[-1], delta=1)

    # arg_max_indices = tf.where(final_mask, indices, -1)
    ...
    # DE MOMENTO LOS TRANSFORMO A INT64 PARA PODER TRABAJARLOS
    result_tensor = tf.cast(result_tensor, dtype=tf.int64)

    # ----------------------------

    # Obtengo los valores mayores a 0 pero con las repeticiones de los valores máximos
    result_tensor_values = tf.gather(valores_result, expanded_indices)

    # ----------------------------
    
    # DE MOMENTO TRANSFORMO A INT64 PARA PODER TRABAJARLOS
    filtered_tensor = tf.cast(filtered_tensor, dtype=tf.int64)
